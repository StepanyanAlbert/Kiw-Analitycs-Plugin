...
class Ga_Stats {
	 ...
	 */
	public static function get_query( $query, $id_view, $date_range = null, $metric = null, $dimension = null ) {
		if ( $query == 'main_chart' ) {
			return self::main_chart_query( $id_view, $date_range, $metric, $dimension );
		} elseif ( $query == 'boxes' ) {
			return self::boxes_query( $id_view );
		} elseif ( $query == 'dashboard_boxes' ) {
			return self::dashboard_boxes_query( $id_view, $date_range, $dimension );
	 ...
	 */
	public static function dashboard_boxes_query( $id_view, $date_ranges, $dimension ) {

        if ( empty( $date_ranges ) ) {
            $date_ranges = self::set_date_ranges( '7daysAgo', 'yesterday', '14daysAgo', '8daysAgo' );
        } else {
            $date_ranges = self::set_date_ranges(  $date_ranges[0], $date_ranges[1] );
        }
		...
		$reports_requests[]	 = array(
			'viewId'			 => $id_view,
			'dateRanges'		 => self::set_date_ranges( $date_ranges[0]['startDate'], $date_ranges[0]['endDate'] ),
			...
			'includeEmptyRows'	 => true,
			'dimensions'		 => self::set_dimensions( $dimension )
		);
		$query				 = array(
			'reportRequests' => $reports_requests
		);
	 ...
	 */
	public static function main_chart_query( $id_view, $date_ranges = null, $metric = null, $dimension = null ) {
		if ( empty( $date_ranges ) ) {
			$date_ranges = self::set_date_ranges( '7daysAgo', 'yesterday', '14daysAgo', '8daysAgo' );
		} else {
			$date_ranges = self::set_date_ranges( $date_ranges[0], $date_ranges[1] );
	...
	public static function get_dashboard_chart( $response_data ) {
		...
		if ( !empty( $response_data ) ) {
			...
			if ( !empty( $rows ) ) {
				foreach ( $rows as $row ) {
                    $chart_data[] = array(
						'day'		 => ( (int) $row[ 'dimensions' ][ 0 ] != 0 && is_int((int)$row[ 'dimensions' ][ 0 ] )
                                        || (strlen($row[ 'dimensions' ][ 0 ]) > 1 && $row[ 'dimensions' ][ 0 ][ 0 ] == '0') )
                                        ? date( 'M j', strtotime( $row[ 'dimensions' ][ 0 ] ) )
                                        : $row[ 'dimensions' ][ 0 ],
