...
class Ga_Admin_Controller extends Ga_Controller_Core {
	...
	const ACTION_KIWI_INVITE = 'ga_action_sharethis_invite';
	...
	public static function ga_action_sharethis_invite() {
		...
		if ( self::verify_nonce( self::ACTION_KIWI_INVITE ) ) {
			...
			if ( !empty( $email ) ) {
				...
				$response	 = Ga_Admin::api_client( Ga_Admin::GA_KIWI_API_ALIAS )->call( 'ga_api_sharethis_user_invite', array( $data ) );
				$errors		 = Ga_Admin::api_client( Ga_Admin::GA_KIWI_API_ALIAS )->get_errors();
	...
	public static function ga_action_update_terms() {
		update_option( Ga_Admin::GA_KIWI_TERMS_OPTION_NAME, true );
	...
	public static function validate_ajax_data_change_post( $post ) {
		...
		if ( self::verify_nonce( 'ga_ajax_data_change' ) ) {
			if ( !empty( $post[ 'date_range_start' ] ) ) {
                if ( !is_string( $post[ 'date_range_start' ] ) ) {
					$error ++;
				}
			} else {
                $error ++;
