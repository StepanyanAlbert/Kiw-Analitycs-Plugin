...
class Ga_Lib_Sharethis_Api_Client extends Ga_Lib_Api_Client {
	...
	const GA_KIWI_ENDPOINT = 'platform-api.sharethis.com/v1.0/property';
	...
	private function ga_api_create_sharethis_property( $query_params ) {
		...
		try {
			$response = $request->make_request( $this->add_protocol( self::GA_KIWI_ENDPOINT ), wp_json_encode( $query_params ), true );
		} catch ( Ga_Lib_Api_Request_Exception $e ) {
	...
	private function ga_api_sharethis_installation_verification( $query_params ) {
		$request = Ga_Lib_Api_Request::get_instance(self::USE_CACHE);
		try {
			$response = $request->make_request( 'https://' . self::GA_KIWI_ENDPOINT . '/verify', wp_json_encode( $query_params ), true );
		} catch ( Ga_Lib_Api_Request_Exception $e ) {
	...
	private function ga_api_sharethis_get_trending_alerts( $query_params ) {
		$url	 = $this->add_protocol( add_query_arg( $query_params, self::GA_KIWI_ENDPOINT . '/notifications' ) );
		$request = Ga_Lib_Api_Request::get_instance(self::USE_CACHE);
		try {
	...
	private function ga_api_sharethis_user_invite( $query_params ) {
		$request = Ga_Lib_Api_Request::get_instance(self::USE_CACHE);
		try {
			$response = $request->make_request( 'https://' . self::GA_KIWI_ENDPOINT . '/user/join', wp_json_encode( $query_params ), true );
		} catch ( Ga_Lib_Api_Request_Exception $e ) {
