...
class Ga_Helper {
	...
	$date_range = null, $metric = null, $dimension = null, $text_mode = false, $ajax = false
	) {
		if ( empty( $date_range ) ) {
		...
		$data = self::get_dashboard_widget_data( $date_range, $metric, $dimension );
	 ...
	 */
	private static function get_dashboard_widget_data( $date_range, $metric = null, $dimension = null ) {
		$selected = self::get_selected_account_data( true );
		if ( self::is_authorized() && self::is_account_selected() ) {
			$query_params = Ga_Stats::get_query( 'main_chart', $selected[ 'view_id' ], $date_range, $metric, $dimension);
			$stats_data	  = Ga_Admin::api_client()->call( 'ga_api_data', array(
				$query_params
			) );

			$boxes_query = Ga_Stats::get_query( 'dashboard_boxes', $selected[ 'view_id' ], $date_range, $metric, $dimension );
	...
	public static function are_terms_accepted() {
		return self::get_option( Ga_Admin::GA_KIWI_TERMS_OPTION_NAME );
	}
	...
	public static function is_sharethis_included() {
		return GA_KIWI_SCRIPTS_INCLUDED;
	}
	...
	public static function are_sharethis_properties_verified() {
		return ( get_option( Ga_Admin::GA_KIWI_VERIFICATION_RESULT ) && self::are_sharethis_properties_set() );
	}

	public static function are_sharethis_properties_ready_to_verify() {
		return ( self::are_sharethis_properties_set() && !get_option( Ga_Admin::GA_KIWI_VERIFICATION_RESULT ) );
	}

	public static function are_sharethis_properties_set() {
		return ( get_option( Ga_Admin::GA_KIWI_PROPERTY_ID ) && get_option( Ga_Admin::GA_KIWI_PROPERTY_SECRET ) );
	}

	public static function is_plugin_version_with_trending_content() {
		return ( version_compare( get_option( Ga_Admin::GA_VERSION_OPTION_NAME ), Ga_Admin::GA_KIWI_TRENDING_CONTENT_PLUGIN_VERSION, '>=' ) );
	}
