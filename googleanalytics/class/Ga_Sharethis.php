...
class Ga_Sharethis {
	...
	const GA_KIWI_ALERTS_ERROR = 'Trending content alerts are temporarily unavailable, please try again later or contact info@kiwi-science.com';
	...
	public static function create_sharethis_options( $api_client ) {
		...
		if ( Ga_Helper::should_create_sharethis_property() ) {
			...
			if ( !empty( $sharethis_options[ 'id' ] ) ) {
				add_option( Ga_Admin::GA_KIWI_PROPERTY_ID, $sharethis_options[ 'id' ] );
			}
			if ( !empty( $sharethis_options[ 'secret' ] ) ) {
				add_option( Ga_Admin::GA_KIWI_PROPERTY_SECRET, $sharethis_options[ 'secret' ] );
			}
		}
	...
	public static function get_sharethis_options( $response ) {
		...
		if ( !empty( $body ) ) {
			...
			$options[ 'error' ] = self::GA_KIWI_ALERTS_ERROR;
		}
		return $options;
	}
	...
	public static function sharethis_installation_verification( $api_client ) {
		if ( Ga_Helper::should_verify_sharethis_installation() ) {
			$query_params	 = array(
				'id'	 => get_option( Ga_Admin::GA_KIWI_PROPERTY_ID ),
				'secret' => get_option( Ga_Admin::GA_KIWI_PROPERTY_SECRET )
			);
			...
			if ( !empty( $result ) ) {
				add_option( Ga_Admin::GA_KIWI_VERIFICATION_RESULT, true );
			}
		}
	}
	...
	public static function get_alerts( $response ) {
		...
		if ( !empty( $body ) ) {
			if ( !empty( $body[ 'error' ] ) ) {
				return (object) array( 'error' => self::GA_KIWI_ALERTS_ERROR );
			}
