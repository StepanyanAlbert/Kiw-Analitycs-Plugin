...
class Ga_Admin {
	...
	const GA_KIWI_TERMS_OPTION_NAME                     = 'googleanalytics_sharethis_terms';
	...
	const GA_KIWI_PROPERTY_ID                           = 'googleanalytics_sherethis_property_id';
	const GA_KIWI_PROPERTY_SECRET                       = 'googleanalytics_sherethis_property_secret';
	const GA_KIWI_VERIFICATION_RESULT                   = 'googleanalytics_sherethis_verification_result';
	const MIN_WP_VERSION                                = '3.8';
	const GA_KIWI_TRENDING_CONTENT_PLUGIN_VERSION       = '2.1';
	const GA_KIWI_API_ALIAS                             = 'sharethis';
	...
	public static function api_client( $type = '' ) {
		if ( self::GA_KIWI_API_ALIAS === $type ) {
	...
	public static function activate_googleanalytics() {
		...
		add_option( self::GA_KIWI_TERMS_OPTION_NAME, false );
	...
	public static function uninstall_googleanalytics() {
		delete_option( self::GA_KIWI_TERMS_OPTION_NAME );
		delete_option( self::GA_HIDE_TERMS_OPTION_NAME );
		delete_option( self::GA_VERSION_OPTION_NAME );
		delete_option( self::GA_KIWI_PROPERTY_ID );
		delete_option( self::GA_KIWI_PROPERTY_SECRET );
		delete_option( self::GA_KIWI_VERIFICATION_RESULT );
	}
	...
	public static function update_googleanalytics() {
		...
		if ( empty( $old_property_value ) && empty( $version ) ) {
			update_option( self::GA_KIWI_TERMS_OPTION_NAME, true );
		}
	...
	public static function admin_menu_googleanalytics() {
		if ( current_user_can( 'manage_options' ) ) {
			add_menu_page( 'Google Analytics', 'Google Analytics', 'manage_options', 'googleanalytics', 'Ga_Admin::statistics_page_googleanalytics', 'dashicons dashicons-chart-bar', 1000 );
...
//			add_submenu_page( 'googleanalytics', 'Google Analytics', __( 'Trending Content' ), 'manage_options', 'googleanalytics/trending', 'Ga_Admin::trending_page_googleanalytics' );
	...
	public static function trending_page_googleanalytics() {
		...
		$data = Ga_Sharethis::create_sharethis_options( self::api_client( self::GA_KIWI_API_ALIAS ) );

		Ga_Sharethis::sharethis_installation_verification( self::api_client( self::GA_KIWI_API_ALIAS ) );

		$alerts = Ga_Sharethis::load_sharethis_trending_alerts( self::api_client( self::GA_KIWI_API_ALIAS ) );
		// Handle invitation result
		$data = Ga_Helper::handle_url_message( $data );
		if ( Ga_Helper::is_wp_old() ) {
			self::display_api_errors( self::GA_KIWI_API_ALIAS );
		}
		Ga_View_Core::load(
			'trending',
			array(
				...
				'errors'  => self::api_client( self::GA_KIWI_API_ALIAS )->get_errors(),
			)
		);
		if ( ! Ga_Helper::is_wp_old() ) {
			self::display_api_errors( self::GA_KIWI_API_ALIAS );
	...
	public static function admin_notice_googleanalytics() {
		if ( ( ! get_option( self::GA_KIWI_TERMS_OPTION_NAME ) && Ga_Helper::is_plugin_page() ) || ( ! get_option( self::GA_KIWI_TERMS_OPTION_NAME ) && ! get_option( self::GA_HIDE_TERMS_OPTION_NAME ) ) ) {
	...
	public static function add_actions() {
		...
		if ( ! get_option( self::GA_KIWI_TERMS_OPTION_NAME ) && ! get_option( self::GA_HIDE_TERMS_OPTION_NAME ) ) {
	...
	public static function ga_ajax_data_change() {
        if ( Ga_Admin_Controller::validate_ajax_data_change_post( $_POST ) ) {
			$date_range_start = ! empty( $_POST['date_range_start'] ) ? $_POST['date_range_start'] : null; // WPCS: CSRF ok.
			$date_range_end = ! empty( $_POST['date_range_end'] ) ? $_POST['date_range_end'] : null; // WPCS: CSRF ok.
            $date_range = [$date_range_start, $date_range_end];
			$metric     = ! empty( $_POST['metric'] ) ? $_POST['metric'] : null; // WPCS: CSRF ok.
            $dimension  = ! empty( $_POST['dimension'] ) ? $_POST['dimension'] : null; // WPCS: CSRF ok.

			echo wp_kses_post( Ga_Helper::get_ga_dashboard_widget_data_json( $date_range, $metric, $dimension, false, true ) );
		} else {
	...
	public static function generate_stats_data() {
		...
		$last_chart_date = isset( $chart['date'] ) ? $chart['date'] : strtotime( 'now' );
